    ...
    <section id="amenities" className="star-field cosmic-gradient py-24">
      <div className="section-shell relative z-10">
        ...
        <ul className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {amenities.map((a, i) => {
            const reveal = useReveal<HTMLLIElement>(i * 80);
            return (
              <li
                key={a.label}
                {...reveal}
                className={`${reveal.className} flex items-center gap-4 rounded-2xl border border-accent/25 bg-primary/30 p-5 backdrop-blur`}
              >
